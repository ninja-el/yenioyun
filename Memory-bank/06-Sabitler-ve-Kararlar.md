...
| Yığın alanı iç ölçüsü | 6 x 9 x 9 (genişlik x yükseklik x derinlik), merkez (0, 4.5, 0); taban y=0 | `StackArea` (GameScene) |
| Alan duvar kalınlığı | 1.0 birim (zemin + 4 duvar + tavan) | `StackArea` |
| Doğma noktaları arası ek boşluk | 0 birim (sınır küreleri değer ama çakışmaz; küre collider merkezine kurulur) | `StackArea` (GameScene) |
| Yığını tabandan doldurma | Açık (her obje denenen adaylar içinde boş olan en alçak noktaya konur) | `StackArea` (GameScene) |
| Boş nokta aramasında aday sayısı | 300 | `StackArea` (GameScene) |
...
| 65 | Tüm sesler `Data/Config/AudioLibrary` asset'inden okunur; klip değiştirmek için yalnızca oradaki alan değiştirilir. Çalma kararı sadece `AudioManager`'dadır ve event dinler: doğru obje `MatchResolver.OnItemMatched`, yanlış obje `OnItemMissed`, kutu kapanması `Conveyor.OnBoxFilled`, makineden kutu çıkması `Conveyor.OnBoxSpawned`, booster `BoosterManager.OnBoosterUsed` (Auto-Match ise ayrıca UFO sesi), win/fail `GameManager.OnLevelCompleted/OnLevelFailed`, müzik oyun durumuna göre menü/oyun. Buton tıklama sesi `AudioManager.Start`'ta MainScene'deki tüm `Button`'lara bağlanır; `BoosterButton` hariçtir (booster sesini çalar, stok yoksa panel açarken tıklama çalar). Ses dosyaları `Audio/SFX/SFX_*`, müzikler `Audio/Music/BGM_*` | Oyuncu isteği. UI butonlarının hepsi MainScene'de ve açılışta hazır; her butona ayrı bileşen eklemek sahnede 50 butona dokunmak demekti. Çalışma anında oluşturulan bir buton olursa bu bağlamaya girmez, kendisi `PlayButtonClick` çağırmalıdır |
